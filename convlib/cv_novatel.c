...
static char* cv_obscodes[] = {       /* observation code strings */
...
static char cv_codepris[7][MAXFREQ][16] = {  /* code priority for each freq-index */
...
static void cv_matcpy(double *A, const double *B, uint32_t n, uint32_t m)
{
...
*-----------------------------------------------------------------------------*/
extern char* code2obs_s(uint8_t code)
{
    if (code <= CODE_NONE || MAXCODE < code) return "";
    return cv_obscodes[code];
}

/* GPS obs code to frequency -------------------------------------------------*/
static int code2freq_GPS(uint8_t code, double* freq)
{
    char* obs = code2obs_s(code);
...
static int code2freq_BDS(uint8_t code, double* freq)
{
    ...
    case '2': *freq = FREQ1_BDS;  return 0; /* B1I */
    case '7': *freq = FREQ2_BDS;  return 1; /* B2I/B2b */
    case '5': *freq = FREQL5;     return 2; /* B2a */
    case '6': *freq = FREQ3_BDS;  return 3; /* B3 */
...
*          uint8_t code     I   obs code (CODE_???)
*          raw_nav_t  *raw_nav_t    I   navigation data for GLONASS (NULL: not used)
* return : carrier frequency (Hz) (0.0: error)
*-----------------------------------------------------------------------------*/
extern double sat2freq(int sat, uint8_t code, const raw_nav_t* nav)
{
	int i, fcn = 0, sys, prn;

	sys = cv_satsys(sat, &prn);
...
static gtime_t adjweek_s(gtime_t time, double tow)
{
    double tow_p;
    int week;
    tow_p=cv_time2gpst(time,&week);
    if      (tow<tow_p-302400.0) tow+=604800.0;
    else if (tow>tow_p+302400.0) tow-=604800.0;
    return cv_gpst2time(week,tow);
}
...
static int obsindex(raw_obs_t *obs, gtime_t time, int sat)
{
    int i,j;
...
static int decode_rangecmpb(raw_t *raw)
{
    ...
    for (i=0,p+=4;i<nobs;i++,p+=24) {
        ...
        if (sys== SYS_QZS &&prn>=MINPRNQZS_S+10&&prn<=MAXPRNQZS_S+10&&code==CODE_L1C) {
            prn-= (MINPRNQZS_S + 9) ;
            //code=CODE_L1Z; /* QZS L1S */
        }
        if (!(sat=cv_satno(sys,prn))) {
        ...
        if (raw->tobs[sat-1][idx].time!=0) {
            tt=cv_timediff(raw->time,raw->tobs[sat-1][idx]);
...
static int decode_rawephemb(raw_t *raw)
{
    ...
    if (!decode_frame(subframe,&eph,NULL,NULL,NULL, NULL)) {
       // trace(2,"oem4 rawephemb subframe error: prn=%d\n",prn);
        return -1;
    }
    eph.sat = sat;
    int i, idx = -1;
    for (i = 0; i < raw->nav.n; i++)
    {
        if (raw->nav.eph[i].sat == sat)
        {
            if (eph.iode == raw->nav.eph[i].iode &&
                eph.iodc == raw->nav.eph[i].iodc) return 0; /* unchanged */
...
static int decode_rawwaasframeb(raw_t *raw)
{
    ...
    raw->sbsmsg.tow=(int)cv_time2gpst(raw->time,&raw->sbsmsg.week);
    raw->sbsmsg.prn=prn;
...
static int decode_gloephemerisb(raw_t *raw)
{
    ...
    geph.age   =U4(p+136);
    geph.toe=cv_gpst2time(week,tow);
    tof+=floor(tow/86400.0)*86400;
    if      (tof<tow-43200.0) tof+=86400.0;
    else if (tof>tow+43200.0) tof-=86400.0;
    geph.tof=cv_gpst2time(week,tof);
    
    /*int i, idx = -1;
    for (i = 0; i < raw->nav.ng; i++)
    {
        if (raw->nav.geph[i].sat == sat)
        {
            if (geph.iode == raw->nav.geph[i].iode && geph.iode > 0) return 0; 
...
static int decode_qzssrawephemb(raw_t *raw)
{
    ...
    if (!decode_frame(subfrm,&eph,NULL,NULL,NULL, NULL)) {
...
static int decode_qzssrawsubframeb(raw_t *raw)
{
    ...
    if (id==3) {
        if (!decode_frame(raw->subfrm[sat-1],&eph,NULL,NULL,NULL, NULL)) return 0;
        ...
        if (!decode_frame(raw->subfrm[sat-1],NULL,NULL,ion,utc, NULL)) return 0;
        adj_utcweek(raw->time,utc);
        cv_matcpy(raw->nav.ion_qzs,ion,8,1);
        cv_matcpy(raw->nav.utc_qzs,utc,8,1);
        return 9;
    }
    return 0;
}
...
static int decode_galephemerisb(raw_t *raw)
{
    ...
    eph.iodc=eph.iode;
    tow=cv_time2gpst(raw->time,&week);
    eph.week=week; /* gps-week = gal-week */
    eph.toe=cv_gpst2time(eph.week,eph.toes);
    
    tt=cv_timediff(eph.toe,raw->time);
    if      (tt<-302400.0) eph.week++;
    else if (tt> 302400.0) eph.week--;
    eph.toe= cv_gpst2time(eph.week,eph.toes);
    eph.toc= adjweek_s(raw->time,set?toc_fnav:toc_inav);
    eph.ttr=raw->time;
...
static int decode_bdsephemerisb(raw_t *raw)
{
    ...
    eph.toe=cv_bdt2gpst(cv_bdt2time(eph.week,eph.toes)); /* bdt -> gpst */
    eph.toc=cv_bdt2gpst(cv_bdt2time(eph.week,toc));      /* bdt -> gpst */
    eph.ttr=raw->time;
...
static int decode_navicephemerisb(raw_t *raw)
{
    ...
    eph.toe=cv_gpst2time(eph.week,eph.toes);
    eph.toc=cv_gpst2time(eph.week,toc);
    eph.ttr=raw->time;
    eph.tgd[1]=0.0;
    ...
    /*if (!strstr(raw->opt, "-EPHALL")) {
        if (timediff(raw->nav.eph[raw->nav.n].toe,eph.toe)==0.0&&
            raw->nav.eph[raw->nav.n].iode==eph.iode) return 0; 
    }
    raw->nav.eph[raw->nav.n]=eph;
    raw->nav.n++;
    raw->ephsat=sat;
...
static int decode_rgeb(raw_t *raw)
{
    ...
    week=cv_adjgpsweek(U4(p));
    //week = U4(p);
    tow =R8(p+ 4);
    nobs=U4(p+12);
    raw->time=cv_gpst2time(week,tow);
    ...
    for (i=0,p+=20;i<nobs;i++,p+=44) {
        ...
        if (!(sat=cv_satno(sys==1?SYS_GLO:(sys==2?SYS_SBS:SYS_GPS),prn))) {
            //trace(2,"oem3 regb satellite number error: sys=%d prn=%d\n",sys,prn);
            continue;
        }
        if (raw->tobs[sat-1][freq].time!=0) {
            tt=cv_timediff(raw->time,raw->tobs[sat-1][freq]);
...
static int decode_rged(raw_t *raw)
{
    ...
    for (i=0,p+=12;i<nobs;i++,p+=20) {
        ...
        if ((index=obsindex(&raw->obs,raw->time,sat))>=0) {
            ...
            raw->obs.data[index].P  [freq]=psr;
            raw->obs.data[index].D  [freq]=(float)dop;
            raw->obs.data[index].SNR[freq]=(uint8_t)(snr/SNR_UNIT+0.5);
            raw->obs.data[index].LLI[freq]=(uint8_t)lli;
            raw->obs.data[index].code[freq]=freq==0?CODE_L1C:CODE_L2P;
        }
    }
...
static int decode_repb(raw_t *raw)
{
    ...
    if (!decode_frame(p+4,&eph,NULL,NULL,NULL, NULL)) {
...
static int decode_utcb(raw_t *raw)
{
    ...
    raw->nav.utc_gps[3]=cv_adjgpsweek(U4(p+20));
    //raw->nav.utc_gps[3] = U4(p + 20);
...
static int decode_oem4(raw_t *raw)
{
   ...
   // trace(3,"decode_oem4: type=%3d len=%d\n",type,raw->len);
    ...
    /* check crc32 */
    if (cv_rtk_crc32(raw->buff,raw->len)!= U4(raw->buff+raw->len)) {
    ...
    week=cv_adjgpsweek(week);
    tow =U4(raw->buff+16)*0.001;
    raw->time=cv_gpst2time(week,tow);
    if (msg!=0) return 0;
    
    if (raw->outtype) {
        cv_time2str(cv_gpst2time(week,tow),tstr,2);
