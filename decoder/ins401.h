...
#pragma pack(push, 1)
	...
	typedef struct {
		uint8_t gnss_err_out_pin : 1;
		uint8_t gnss_rf_err_pin : 1;
		uint8_t gnss_ant_err_pin : 1;
		uint8_t gnss_handshake_flag : 1; // 1 ok ,0 failure
		uint8_t gnss_reset_pin : 1;
		uint8_t reserved : 3;
	} gnss_pin_bit;
    ...
    typedef struct  heading
    {
        ...
        float       gps_millisecs;     // Milliseconds into week
        float       length;
        float       heading;
        float       pitch;
        float       roll;
