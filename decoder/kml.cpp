...
void Kml_Common::write_header(FILE * kml_file, int ntype)
{
	if (kml_file) {
...
void Kml_Common::write_end(FILE * kml_file)
{
	if (kml_file) {
		fprintf(kml_file, "</Document>\n");
...
void Kml_Ins_Obj::set_kml_frequency(int frequency)
{
	ins_kml_frequency = frequency;
}

void Kml_Ins_Obj::set_status_type_define(emStatusTypeDefine define)
{
	status_type_define = define;
}

void Kml_Ins_Obj::open_file(const char * file_base_name, const char * filename)
{
	char file_name[1024] = { 0 };
	if (ins_sol_list.size() > 0) {
		sprintf(file_name, "%s_%s", file_base_name, filename);
		f_ins_kml = fopen(file_name, "wb");
	}
}

void Kml_Ins_Obj::write_file()
{
	if (f_ins_kml) {
		Kml_Common::write_header(f_ins_kml, 1);
		...
		ins_sol_list.clear();
		Kml_Common::write_end(f_ins_kml);
	}
}

void Kml_Ins_Obj::close_file()
{
	if (f_ins_kml)fclose(f_ins_kml); f_ins_kml = NULL;
}

void Kml_Ins_Obj::append_ins(kml_ins_t & ins)
{
...
int Kml_Ins_Obj::get_ins_status_color(int status)
{
	int color = 0;
...
void Kml_Ins_Obj::write_ins_line(kml_ins_t * ins, int begin_end)
{
	if (f_ins_kml) {
		if (begin_end == 1) {
...
void Kml_Ins_Obj::write_ins_point(kml_ins_t * ins, int begin_end)
{
	if (f_ins_kml) {
		if (begin_end == 1) {
		...
		}
		{
			...
			if (kml_description) {
				...
				if (status_type_define == emAceinnaStatusType) {
					if (ins->ins_status <= 5 && ins->ins_position_type <= 5) {
						...
						fprintf(f_ins_kml, "<TR ALIGN=RIGHT><TD ALIGN=LEFT>Mode:</TD><TD>%s</TD><TD>%d</TD></TR>\n",
							ins_status[ins->ins_status], ins->ins_position_type);
					}
					else if (ins->ins_position_type <= 5) {
						fprintf(f_ins_kml, "<TR ALIGN=RIGHT><TD ALIGN=LEFT>Mode:</TD><TD>%d</TD><TD>%s</TD></TR>\n",
							ins->ins_status, ins_postype[ins->ins_position_type]);
					}
					else {
...
{
	if (f_gnss_kml) {
		Kml_Common::write_header(f_gnss_kml, 0);
		...
		gnss_sol_list.clear();
		Kml_Common::write_end(f_gnss_kml);
	}
}
