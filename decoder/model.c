...
extern int print_nmea_gga(double *ep, double *xyz, int nsat, int type, 
...
extern void RealToArray(double real, char *a, unsigned char id, unsigned char dd) 
...
extern char *itoa_user(int num, char *str, int radix)
{ 
...
extern int print_nmea_gst(double *ep, float *var_llh, char *buff)
{
	char *p = (char *)buff, *q, sum;

	p += sprintf(p, "$GNGST,%02.0f%02.0f%05.2f,%2.1f,%3.1f,%3.1f,%3.1f,%3.1f,%3.1f",
				     ep[3], ep[4], ep[5], 0.0, 0.0, 0.0, var_llh[1], var_llh[0], 
		             var_llh[2]);
	for (q = (char *)buff + 1, sum = 0; *q; q++)
		sum ^= *q; /* check-sum */
